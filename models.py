...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, num_classes, img_dim=415):
        super(YOLOLayer, self).__init__()
        self.anchors = anchors
        self.num_classes = num_classes  # 类别个数，voc为20，coco为80
        self.num_anchors = len(anchors)  # anchor数量，一般为3
        self.ignore_thres = 0.5
        self.mse_loss = nn.MSELoss()
        self.bce_loss = nn.BCELoss()
        self.obj_scale = 1  # λ obj
        self.noobj_scale = 100  # λ noobj
        self.metrics = []
        self.img_dim = img_dim  # 图片高度
        self.grid_size = 0
    ...
    def compute_grid_offset(self, grid_size, cuda=True):
        ...
        g = self.grid_size
        FloatTensor = torch.FloatTensor if not cuda else torch.cuda.FloatTensor
        # 当前一个点代表原图的像素
        ...
        self.scaled_anchors = FloatTensor(
            [(aw / self.stride, ah / self.stride) for aw, ah in self.anchors]
        )
        # 分别获取当前的锚框宽高
    ...
    def build_targets(self, pred_boxes, pred_cls, target, anchors, ignore_thres, cuda):
        ByteTensor = torch.ByteTensor if not cuda else torch.cuda.ByteTensor
        FloatTensor = torch.FloatTensor if not cuda else torch.cuda.FloatTensor
        nB = pred_boxes.size(0)  # 样本数量
        nA = pred_boxes.size(1)  # anchor数量
        nC = pred_cls.size(-1)  # 类别得数量
        nG = pred_boxes.size(2)  # Grid得大小 13 26 52
        shape = (nB, nA, nG, nG)
        obj_mask = ByteTensor(*shape).fill_(0)  # 有物体mask
        noobj_mask = ByteTensor(*shape).fill_(1)  # 无物体mask
        [cls_mask, iou_scores, tx, ty, tw, th] = [
            FloatTensor(*shape).fill_(0) for i in range(6)
        ]  # 预先设定变量
        tcls = FloatTensor(nB, nA, nG, nG, nC).fill_(0)
        target_boxes = target[:, 2:6] * nG  # 当前尺度下，目标框的中心坐标和宽高
        gxy = target_boxes[:, :2]  # 中心点
        gwh = target_boxes[:, 2:]  # 宽高
        # 计算每个点，三个预测框的 iou
        ious = torch.stack([self.bbox_wh_iou(anchor, gwh) for anchor in anchors])
        # 获取最高的iou和对应的框的index
        best_ious, best_n = ious.max(0)

        b, target_labels = target[:, :2].long().t()  # 获取target，对应的图的序号和label
        gx, gy = gxy.t()  # groud truth x ,y
        gw, gh = gwh.t()  # groud truth w ,h
        gi, gj = gxy.long().t()  # 获取整数部分，中心坐标对应格子的位置
        obj_mask[b, best_n, gj, gi] = 1  # 记录obj mask
        noobj_mask[b, best_n, gj, gi] = 0  # 记录 noobj mask

        for i, anchor_ious in enumerate(ious.t()):
            # 对图中iou 大于指定ignore_thres 的noobj_mask 忽略 ？ 为什么obj_mask 这里不置为1
            noobj_mask[b[i], anchor_ious > ignore_thres, gj[i], gi[i]] = 0

        tx[b, best_n, gj, gi] = gx - gx.floor()  # 相对左上角的偏离
        ty[b, best_n, gj, gi] = gy - gy.floor()  # 相对左上角的偏离
        tw[b, best_n, gj, gi] = torch.log(gw / anchors[best_n][:, 0] + 1e-16)
        th[b, best_n, gj, gi] = torch.log(gh / anchors[best_n][:, 1] + 1e-16)

        tcls[b, best_n, gj, gi, target_labels] = 1  # label mask

        cls_mask[b, best_n, gj, gi] = (
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        if self.grid_size == 0:
            # 初始化计算所需的参数
            self.compute_grid_offset(grid_size, cuda)
    ...
    def forward(self, x, target=None):
        ...
        for i, (module_def, module) in enumerate(
            ...
            if module_def["type"] in {"convolutional", "upsample", "maxpool"}:
                ...
                layers_i = int(module_def["from"])
                x = layer_outputs[-1] + layer_outputs[layers_i]
...
def create_modules(module_defs):
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            modules.add_module(f"max_pool_{module_i}", maxpool)
